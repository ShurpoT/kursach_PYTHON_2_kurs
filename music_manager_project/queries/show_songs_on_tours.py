...
from db import fetch_songs_on_tours  # Добавляем импорт метода для запроса
...
def show_songs_on_tours(frame, connection):
    """
    Отображение информации о песнях, исполняемых группой на гастролях.
    """
    ...
    def on_search(group_name_entry):
        ...
        if group_name:
            # Получаем данные о песнях, исполняемых группой на гастролях
            data = fetch_songs_on_tours(connection, group_name)
            
            if not data:
                message = tk.Label(result_frame, text="Нет данных для выбранной группы.", font=("Arial", 14), fg="red")
                message.pack(pady=20)
                return

            # Создаем таблицу с результатами
            columns = ("Песня", "Город", "Дата начала", "Дата окончания")
